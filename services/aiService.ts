...
2.  **Mermaid.js Diagrams:**
    ...
    -   **CRITICAL Mermaid Formatting Rules:**
        -   ALL labels, texts, and descriptions in Mermaid diagrams MUST be enclosed in double quotes ("") to prevent rendering issues with special characters
        -   ALL arrows must use left-to-right notation (LR) for consistency: use \`-->\` instead of \`<--\`, \`-->>\` instead of \`<<--\`, etc.
        -   Direction declarations must use \`LR\` (Left to Right): \`graph LR\`, \`flowchart LR\`, etc.
